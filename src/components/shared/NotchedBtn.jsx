...
const NOTCH = 14 // px — must match --notch in CSS
...
export default function NotchedBtn({ href, onClick, type = "button", variant = "filled-gold", children }) {
    ...
    if (isOutline) {
        ...
        const btn = href ? (
            <a href={href} ref={btnRef} className={cls}>
                {inner}
            </a>
        ) : (
            <button type={type} onClick={onClick} ref={btnRef} className={cls}>
                {inner}
            </button>
        )
        ...
        return (
            <span className="notched-btn-wrap">
                {btn}
                {size.w > 0 && (
                    <svg className="notched-btn__svg" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <polygon points={pts(size.w, size.h)} fill="none" stroke={STROKE[variant] ?? "currentColor"} strokeWidth="1.5" vectorEffect="non-scaling-stroke" />
                    </svg>
                )}
            </span>
        )
    ...
    const inner = ["filled-gold", "filled-dark", "filled-ivory"].includes(variant) ? <span className="notched-btn__text">{children}</span> : children
